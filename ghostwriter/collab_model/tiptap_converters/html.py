...
from ghostwriter.collab_model.tiptap_converters.base import NoHandlerException, TiptapTagHandlerMixin, TiptapMarkHandlerMixin
import pycrdt
...
class TiptapToHtml(
    TiptapTagHandlerMixin[None, Element, set[str]],
    TiptapMarkHandlerMixin[None, Element, set[str]],
):
    """
    ...
    def _apply_text_formatting(
        ...
        for name, value in changes:
            try:
                self.dispatch_mark(name, value, node, classes)
            except NoHandlerException as e:
                logger.warning("Unrecognized mark: %r", e.name)
    ...
    def _apply_element_formatting(
        ...
        classes = set(node.getAttribute("class").split())
        try:
            self.dispatch_tag(yxml_tag, changes, node, classes)
        except NoHandlerException as e:
            logger.warning("Unimplemented yxml tag: %s", e.name)
            node.tagName = "div"
    ...
    def apply_mark_link(
        self, attr: Any | None, el: Element, classes: set[str]
    ) -> None:
        if attr is None:
            el.tagName = "span"
    ...
    def apply_mark_bold(
        self, attr: Any | None, _el: Element, classes: set[str]
    ) -> None:
    ...
    def apply_mark_italic(
        self, attr: Any | None, _el: Element, classes: set[str]
    ) -> None:
        modify_class(classes, "italic", attr)

    def apply_mark_strike(
        self, attr: Any | None, _el: Element, classes: set[str]
    ) -> None:
        modify_class(classes, "strike", attr)

    def apply_mark_underline(
        self, attr: Any | None, _el: Element, classes: set[str]
    ) -> None:
